...
### 建築図面PDFをAI向けに索引化するためのエージェント支援スキルセット

建築図面 PDF を読み取り、AI エージェントや検索エンジン、RAG（検索拡張生成）システムが情報を処理しやすいよう、SQLite や JSONL 形式のインデックス（索引）へ構造化・変換するためのワークフロー雛形（リファレンス実装）です。

**【注意】** 本リポジトリは、あらゆる図面PDFを完全自動で解析する完成品ではありません。PDFテキスト抽出、OCR、目次推定、SQLite/JSONL出力の基本実装と、現場ごとの読み取り要望をAIエージェントに伝えるための運用テンプレートを提供します。タイトル枠の抽出は末尾数十行に対する正規表現ベースのヒューリスティックであり、目次解析も特定の複数列レイアウトを想定しています。そのため、導入先の設計フォーマットや図面枠のテンプレートに合わせて、正規表現パターン（`FIELD_PATTERNS`等）やプロンプトなどを適宜調整・カスタマイズしていただく必要があります。

`zumen-indexer` は、図面原本とAIエージェント間のギャップを埋めるためのベースライン（雛形）を提供します。コマンドライン（CLI）からの実行はもちろん、Pythonモジュールとして他のシステムやAIエージェントのインデックス生成ワークフローに直接組み込んで利用することも可能です。
...
図面の線画（ビジュアル）からのみ判定可能な情報（キッチンの左右勝手など）をAIに読み取らせたい場合は、リポジトリ内の [visual_extraction_checklist.md](visual_extraction_checklist.md) をテンプレートとして現場や配布先に提供してください。

* **運用方法:**
  1. **テンプレートのコピー**:
     図面インデックスを作成する前に、[visual_extraction_checklist.md](visual_extraction_checklist.md) をコピーし、対象の図面セット専用のチェックリストファイルを作成します。
