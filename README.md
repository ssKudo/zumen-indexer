...
建築図面 PDF を読み取り、AI エージェントや検索エンジン、RAG（検索拡張生成）システムが情報を処理しやすいよう、SQLite や JSONL 形式のインデックス（索引）へ構造化・変換するための支援ツールです。

**【注意】** 本ツールは、あらゆる建築図面を完全自動かつ万能にパースする魔法のツールではありません。タイトル枠の抽出は「末尾数行に対する正規表現パターンマッチ」をベースとし、目次の解析も「特定の複数列レイアウト」を想定したヒューリスティック（経験則）抽出器です。導入先の設計フォーマットや図面枠のテンプレートに合わせて、正規表現パターン（`FIELD_PATTERNS`等）やプロンプトなどを適宜調整していただく必要があります。

`zumen-indexer` は、図面原本とAIエージェント/スクリプト間のギャップを埋めるためのベースラインを提供します。コマンドライン（CLI）からの実行はもちろん、Pythonモジュールとして他のシステムやAIエージェントのインデックス生成ワークフローに直接組み込んでカスタマイズして利用することも可能です。
